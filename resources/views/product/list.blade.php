...
@section('content')
    <div class="row">
		<table id="data-csgoroll" class="table table-striped table-bordered table-sm table-hover" cellspacing="0" width="100%">
		    <thead>
		        <tr>
		            <th scope="col">STT</th>
		    ...
		    <tbody>
		    	...
		    	@foreach($csgoroll as $item)
		    		<tr>
			            <td>{{$i++}}</td>
			            <td>
			            	<a target="_blank" href="{{ route('product.detail', ['requestID' => $requestID, 'name' => $item->full_name]) }}">
			            		{{$item->full_name}}
			            	</a> 
			            </td>
			            <td 
			            	@if($item->buff)
			            		data-toggle="tooltip" data-placement="top" title="{{$item->buff}}"
		            		@endif
			            >
			            	@if($item->buff)
			            		{{number_format($item->buff/$item->conversion_price_buff, 2)}}
		            		@endif
			            </td>
			            <td 
